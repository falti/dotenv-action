...
test('test runs with bypass case for keys', () => {
    const dotenv_action = require('./dotenv_action');
    expect(dotenv_action("fixtures/.case.env", "bypass")).toEqual({ "FIXTURES_4": "foo", "fixtures_5": "bar", "Fixtures_6": "abc" });
})

test('test runs with lower case for keys', () => {
    const dotenv_action = require('./dotenv_action');
    expect(dotenv_action("fixtures/.case.env", "lower")).toEqual({ "fixtures_4": "foo", "fixtures_5": "bar", "fixtures_6": "abc" });
})

test('test runs with upper case for keys', () => {
    const dotenv_action = require('./dotenv_action');
    expect(dotenv_action("fixtures/.case.env", "upper")).toEqual({ "FIXTURES_4": "foo", "FIXTURES_5": "bar", "FIXTURES_6": "abc" });
})